...
export function HeroImageSlider({ imageUrls, className }: HeroImageSliderProps) {
  ...
  const showArrows = urls.length > 4; // 요구사항 유지: 4개 초과일 때만
  const maxStart = Math.max(0, urls.length - perView);

  const goPrev = () => setCurrent((c) => Math.max(0, c - perView));
  const goNext = () => setCurrent((c) => Math.min(maxStart, c + perView));
  ...
  return (
    <>
      ...
      <div className={`relative ${className || ""}`}>
        <div
          className="overflow-hidden w-full max-h-96"
        ...
        >
          ...
          >
            {urls.map((src, idx) => (
              <div
                key={`${src}-${idx}`}
                className="relative flex-none aspect-[1/1] overflow-hidden cursor-pointer"
