...
import { SUPPORT_LANG_CODES, TGroupDetail } from "dplus_common_v1";

export function HeroImageBackgroundCarouselGroup({ bucket, imageUrls = [], interval = 5000, groupDetail, langCode }: {
  bucket: string;
  imageUrls: string[];
  interval?: number;
  groupDetail: TGroupDetail | null;
      ...
      {groupDetail && (
        <div className={clsx(
          ...
          <div id="stag-title" className="text-center font-extrabold text-4xl md:text-5xl lg:text-6xl"
            data-group-code={groupDetail.group_code}
          >
            <div>{groupDetail.name}</div>
            <div>{groupDetail.name_native}</div>
          </div>
        </div>
      )}
    </div>
