...
export const CompEventActionButtons = (
  ...
  return (
    // <div className="my-6 flex flex-row flex-wrap gap-2 sm:gap-4 justify-center items-center">
    <div className="my-12 flex flex-col gap-4 sm:flex-row sm:flex-wrap items-center justify-center">
      ...
      <button className="cursor-pointer w-full sm:min-w-[160px] sm:max-w-[200px] group">
        <div className="flex items-center justify-center gap-3 border border-gray-900 bg-gray-900 text-white rounded-full py-4 hover:opacity-90 transition-all duration-300">
          <div className="flex items-center justify-center gap-2 group-hover:scale-110 transition-all duration-300">
            {deviceType === 'ios' ? <IconApple className="w-5 h-5" /> : <CalendarDays className="w-5 h-5" />}
            <span className="text-sm font-bold">{deviceType === 'ios' ? getDplusI18n(langCode as (typeof SUPPORT_LANG_CODES)[number]).detail.apple_calendar : getDplusI18n(langCode as (typeof SUPPORT_LANG_CODES)[number]).detail.ics_download}</span>
          </div>
        </div>
      </button>
