...
export default function CompTodayDetailPage({
  ...
  const fetchAndMergeData = async (restoredRawEvents?: TEventCardForDateDetail[]) => {
    ...
    try {
      const res = await clientReqGetTodayList(countryCode, 0, LIST_LIMIT.default);
  ...
  const loadMoreEvents = async () => {
    ...
    try {
      const res = await clientReqGetTodayList(countryCode, eventsStart, LIST_LIMIT.default);
      if (reqId !== requestIdRef.current) return;
