...
import { clientReqGetGroupDetail, clientReqGetGroupEvents } from "@/api/group/clientReqGroup";
import {
  LIST_LIMIT,
...
export default function CompGroupDetailPage({
  ...
  const fetchAndMergeData = async (restoredEvents?: TMapGroupEventWithEventInfo[]) => {
    ...
    try {
      const res = await clientReqGetGroupDetail(groupCode, langCode, 0, LIST_LIMIT.default);
  ...
  const loadMoreEvents = async () => {
    ...
    try {
      const res = await clientReqGetGroupEvents(groupCode, eventsStart, LIST_LIMIT.default);
