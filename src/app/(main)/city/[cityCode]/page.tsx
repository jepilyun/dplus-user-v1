...
import { reqGetCityCodes, reqGetCityMetadata } from "@/actions/action";
...
export async function generateMetadata(
  { params }: { params: { cityCode: string; langCode?: string } }
): Promise<Metadata> {
  const { langCode } = getRequestLocale({ langCode: params?.langCode });
  const dict = getDplusI18n(langCode);

  // API 호출 (에러 대비 안전가드)
  const api = await reqGetCityMetadata(params.cityCode, langCode).catch(() => null);
